...
\begin{itemize}
  ...
  \item Validaci\'on cuantitativa de equivalencia entre modalidad remota y presencial mediante 70 ensayos con m\'etricas de correlaci\'on de Pearson ($r = 0.61$--$0.62$), aceleraci\'on experimental y tasa de captura de eventos (98.5\%).
...
La Figura~\ref{fig:corr_s2} presenta la gráfica de dispersión para S2, con un coeficiente de correlación de Pearson moderado ($r = 0.61$, $R^2 = 0.37$, $N = 35$). Este valor refleja una asociación lineal positiva estadísticamente significativa entre ambas modalidades. La dispersión observada es coherente con la independencia física de los ensayos: dado que cada par (presencial, remoto) corresponde a eventos físicos distintos sujetos a diferentes condiciones iniciales de fricción y liberación del carrito, una correlación perfecta ($r \approx 1$) sería físicamente imposible. El valor $r = 0.61$ con medias prácticamente idénticas ($\bar{x}_P = 1.17$~s vs.\ $\bar{y}_R = 1.19$~s, $\Delta\bar{x} = 0.02$~s) confirma que el sistema IoT captura la variabilidad natural del MRUA sin introducir sesgo sistemático.
...
La Figura~\ref{fig:corr_s3} muestra la gráfica de dispersión para S3, con $r = 0.62$, $R^2 = 0.39$, $N = 35$: el coeficiente más alto de la serie, indicando una correlación moderada positiva. Este resultado es consistente con el mayor rango temporal del sensor ($\bar{x}_P = 1.66$~s), que amplifica la variabilidad física acumulada del MRUA y enriquece la señal estadística. Las desviaciones estándar prácticamente idénticas ($\sigma_P = 0.23$~s vs.\ $\sigma_R = 0.24$~s, $\Delta\sigma = 0.01$~s) demuestran que la detección automatizada por interrupciones de hardware del módulo de control replica la dispersión del cronometraje manual con una fidelidad estadísticamente equivalente.

\subsection{Sensor S4: Final de la pista}

El Sensor S4 representa el punto de medici\'on final, donde la velocidad es m\'axima. Los resultados muestran un excelente acuerdo entre ambas modalidades, con tiempos medios pr\'acticamente id\'enticos (2.05 s vs 2.04 s) y desviaciones est\'andar comparables (0.28 s vs 0.29 s). 
\begin{table}[H]
...
La Figura~\ref{fig:accel_box} presenta la distribuci\'on de la aceleraci\'on experimental estimada mediante la Ecuaci\'on~\ref{eq:aceleracion} para los 35 ensayos de cada modalidad. Ambas distribuciones exhiben medianas y rangos intercuart\'ilicos comparables, con medias id\'enticas ($\mu = 0.98$~m/s$^2$ para ambas modalidades) y desviaciones est\'andar similares ($\sigma = 0.26$~m/s$^2$ remoto vs. $\sigma = 0.28$~m/s$^2$ presencial). Este resultado constituye la evidencia m\'as s\'olida de la equivalencia t\'ecnica del sistema: la instrumentaci\'on IoT y el cronometraje manual producen estimaciones de la magnitud f\'isica central del experimento estad\'isticamente indistinguibles.
\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\linewidth]{figures/acceleration_boxplot.png}
  \caption{Distribuci\'on de la aceleraci\'on experimental estimada para las modalidades remota ($\mu = 0.98$~m/s$^2$, $\sigma = 0.26$~m/s$^2$) y presencial ($\mu = 0.98$~m/s$^2$, $\sigma = 0.28$~m/s$^2$). Los diagramas de caja muestran mediana, rango intercuart\'ilico y valores extremos para $n = 35$ ensayos por modalidad.}
  \label{fig:accel_box}
\end{figure}

La Figura~\ref{fig:mrua_teorico} compara los datos experimentales de posici\'on vs. tiempo con el modelo te\'orico del MRUA definido por la Ecuaci\'on~\ref{eq:mruateorico}. El modelo te\'orico ($a \approx 0.92$~m/s$^2$) se calcul\'o a partir del \'angulo de inclinaci\'on medido de la pista. Los puntos experimentales de ambas modalidades se sit\'uan en la vecindad de la curva te\'orica, con barras de error horizontales que reflejan la dispersi\'on temporal en cada sensor. La aceleraci\'on experimental media ($\mu = 0.98$~m/s$^2$) supera levemente el valor te\'orico en un 6.5\%, diferencia atribuible a imprecisiones en la medici\'on del \'angulo $\theta$ y a efectos de inercia rotacional del carrito no contemplados en el modelo simplificado.
...
Es importante destacar que los coeficientes de correlaci\'on de Pearson obtenidos ($r = 0.61$--$0.62$) no deben interpretarse como una limitaci\'on del sistema IoT. Los valores de $r = 0.61$--$0.62$ son consistentes con lo esperado para series independientes no emparejadas que comparten la misma fuente de variabilidad f\'isica: la naturaleza estoc\'astica del lanzamiento manual del carrito introduce dispersi\'on id\'entica en ambas modalidades, lo que limita el coeficiente de correlaci\'on sin afectar la equivalencia de las distribuciones~\cite{giavarina2015}. El an\'alisis Bland--Altman (Tabla~\ref{tab:bland_altman}) confirma que los l\'imites de concordancia al 95\% son inferiores a 0.08~s, lo que resulta t\'ecnicamente aceptable para el prop\'osito pedag\'ogico del experimento.
...
\begin{adjustwidth}{-\extralength}{0cm}
	...
	\begin{tabular}{p{3.2cm}p{2.0cm}p{2.2cm}p{3.2cm}p{1.4cm}p{1.6cm}p{1.2cm}p{1.2cm}}
		\toprule
		\textbf{Trabajo} & \textbf{Estrategia} & \textbf{Hardware} & \textbf{Validaci\'on remota vs.\ presencial} & \textbf{Coste (USD)} & \textbf{Recursos limitados} & \textbf{Web nativo} & \textbf{C\'od.\ abierto} \\
		\midrule
		Este trabajo                               & Retrofitting   & ESP32 + RPi~4   & S\'i (cuantitativa) & $<$150      & S\'i    & S\'i    & S\'i    \\
		Lustig \textit{et al.}~\cite{r10}          & Retrofitting   & ESP32 + RPi     & No                  & $\sim$200   & Parcial & S\'i    & S\'i    \\
		P\'erez-Cham\'e \textit{et al.}~\cite{r11} & Nuevo hardware & ESP32           & No                  & $\sim$100   & S\'i    & No      & Parcial \\
		Guerrero-Osuna \textit{et al.}~\cite{r4}   & Nuevo sistema  & Arduino + cloud & No                  & $>$500      & No      & S\'i    & No      \\
		Viswanadh \textit{et al.}~\cite{r2}        & Retrofitting   & RPi + sensores  & No                  & $\sim$200   & S\'i    & Parcial & No      \\
		Fuertes \textit{et al.}~\cite{r3}          & Nuevo sistema  & Embebido + nube & No                  & $>$500      & No      & S\'i    & No      \\
		\bottomrule
	\end{tabular}
\end{adjustwidth}
\end{table}
...
El sistema de \textit{retrofitting} IoT propuesto demostr\'o reproducir las mediciones del MRUA con una precisi\'on estad\'isticamente equivalente al montaje presencial de referencia. Las diferencias de medias entre modalidades fueron inferiores a 0.03~s en los cuatro puntos de medici\'on, con $\Delta\sigma < 0.02$~s en todos los sensores (Tabla~\ref{tab:resumen_global}). El resultado m\'as significativo es la igualdad de la aceleraci\'on experimental media en ambas modalidades ($\mu = 0.98$~m/s$^2$, $n = 35$ cada una), con una desviaci\'on del 6.5\% respecto al valor te\'orico esperado ($a \approx 0.92$~m/s$^2$), atribuible a imprecisiones de montaje y no al sistema de medici\'on IoT (Figura~\ref{fig:accel_box}). La tasa de captura exitosa de eventos fue del 98.5\% en modalidad remota, con una \'unica p\'erdida de evento documentada en las 2,380 transmisiones MQTT ejecutadas.

La contribuci\'on central de este trabajo es la validaci\'on cuantitativa, mediante m\'etricas de correlaci\'on de Pearson ($r = 0.61$--$0.62$), perfiles de velocidad y distribuciones de aceleraci\'on, de que un sistema IoT de bajo coste ($<$~USD~150) basado en tecnolog\'ia Edge computing (ESP32) puede replicar las capacidades de medici\'on de un montaje presencial de f\'isica cinem\'atica. La arquitectura de tres capas propuesta, con c\'odigo fuente abierto disponible en GitHub (RemotePhysicsLab~\cite{r15}), es directamente replicable y adaptable a otros experimentos y contextos institucionales de recursos limitados.
