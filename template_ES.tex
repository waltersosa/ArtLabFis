...
\corres{Correspondencia: manuel.nevarez@pucese.edu.ec (M.R.N.T.)}

%-------------------------------------------------
% RESUMEN
%-------------------------------------------------

\abstract{La modernizaci\'on de laboratorios de f\'isica en contextos de recursos limitados presenta desaf\'ios significativos para la educaci\'on universitaria en Am\'erica Latina, agravados por la necesidad de modalidades h\'ibridas y remotas de experimentaci\'on. Este art\'iculo presenta el dise\~no, implementaci\'on y validaci\'on t\'ecnica de un prototipo de laboratorio remoto de cinem\'atica para el estudio del Movimiento Rectil\'ineo Uniformemente Acelerado (MRUA), basado en una estrategia de \textit{retrofitting} con tecnolog\'ias de Internet de las Cosas (IoT) sobre equipamiento preexistente de bajo coste. La arquitectura propuesta integra un microcontrolador ESP32 como nodo de procesamiento en el borde (Edge computing), cuatro sensores infrarrojos, un broker MQTT sobre Raspberry Pi~4 y una interfaz web desarrollada en Next.js/React para control y visualizaci\'on en tiempo real. Para validar el sistema, se ejecutaron 70 ensayos independientes (35 en modalidad remota y 35 en modalidad presencial). Los resultados demuestran que el prototipo remoto alcanza una precisi\'on cinem\'atica comparable al montaje tradicional: las diferencias de medias entre modalidades son inferiores a 0.01~s en los cuatro puntos de medici\'on, las desviaciones est\'andar difieren en menos de 0.02~s ($\Delta\sigma < 0.02$~s), y la aceleraci\'on experimental media es id\'entica en ambas modalidades ($\mu \approx 0.95$~m/s$^2$, con valor te\'orico esperado de $a \approx 0.92$~m/s$^2$). El an\'alisis de Bland--Altman confirma un sesgo medio pr\'acticamente nulo entre modalidades ($|\overline{d}| < 0.002$~s), y la amplitud de los l\'imites de concordancia ($\pm$0.46--0.80~s) es atribuible a la variabilidad f\'isica inherente a ensayos no emparejados, no a limitaciones instrumentales. La tasa de captura exitosa de eventos alcanz\'o el 98.5\% en modalidad remota. Se concluye que la metodolog\'ia de \textit{retrofitting} IoT es una soluci\'on viable, escalable y de bajo coste para democratizar el acceso a la educaci\'on experimental de calidad en entornos h\'ibridos, sin requerir sustituci\'on del equipamiento existente.}
...
\subsection{Fundamento te\'orico del experimento MRUA}

El experimento implementado estudia el Movimiento Rectil\'ineo Uniformemente Acelerado (MRUA) sobre un plano inclinado de \'angulo $\theta$. La aceleraci\'on te\'orica esperada para un carrito de baja fricci\'on se define como:
\begin{equation}
  a_{\text{te\'orico}} = g \cdot \sin\theta
  \label{eq:mruateorico}
\end{equation}
donde $g = 9.81$~m/s$^2$. Los cuatro sensores infrarrojos, distribuidos a intervalos fijos de $\Delta d = 0.532$~m a lo largo de la pista de 1.60~m, registran los instantes de paso del carrito $t_1, t_2, t_3, t_4$. La velocidad media en cada segmento entre sensores consecutivos $i$ e $i+1$ se calcula como:
\begin{equation}
  \bar{v}_{i,i+1} = \frac{\Delta d}{t_{i+1} - t_i}
  \label{eq:velocidad}
\end{equation}
La aceleraci\'on media del sistema se estima por regresi\'on lineal sobre los valores de $\bar{v}$ en funci\'on del tiempo medio de cada segmento:
\begin{equation}
...
La Tabla~\ref{tab:resumen_global} sintetiza los resultados estad\'isticos globales para cada sensor, comparando la modalidad presencial con la remota. Para cada punto de medici\'on se reportan la media ($\bar{x}_P$, $\bar{y}_R$), la desviaci\'on est\'andar ($\sigma_P$, $\sigma_R$) y el coeficiente de correlaci\'on de Pearson ($r$). Esta tabla permite evaluar de forma directa la consistencia y la equivalencia entre las mediciones obtenidas por el montaje presencial de referencia y las registradas por el sistema IoT propuesto.

\begin{table}[H]
\caption{Resumen estad\'istico comparativo por sensor: modalidad presencial vs.\ remota ($n = 35$ por modalidad).}
...
\end{tabular}
\par\smallskip
{\footnotesize\noindent\textit{Nota:} $\bar{x}_P$: media presencial; $\bar{y}_R$: media remota; $\Delta\bar{x}$: diferencia de medias; $r$: coeficiente de correlaci\'on de Pearson. S1 act\'ua como referencia de sincronizaci\'on (varianza nula).}
\end{table}
...
El Sensor S1 corresponde al punto de partida del movimiento ( t= 0). En ambas modalidades, este sensor act\'ua como disparador temporal del experimento; por lo tanto, el tiempo registrado es sistem\'aticamente cero o muy cercano a cero.
...
El Sensor S2 se encuentra al final de la primera sección de la pista. Los resultados en la Tabla~\ref{tab:s2_results} muestran una alta consistencia entre las medias de ambas modalidades (1.09~s vs 1.09~s), con desviaciones est\'andar comparables (0.15~s vs 0.16~s).
...
El Sensor S3 captura el movimiento a una velocidad mayor. La Tabla~\ref{tab:s3_results} muestra tiempos medios muy similares para ambas modalidades (1.54~s vs 1.54~s), con desviaciones est\'andar comparables (0.22~s vs 0.22~s) que indican una precisión de medición consistente en ambos modos.
...
El Sensor S4 representa el punto de medici\'on final, donde la velocidad es m\'axima. Los resultados muestran un excelente acuerdo entre ambas modalidades, con tiempos medios pr\'acticamente id\'enticos (1.89~s vs 1.89~s) y desviaciones est\'andar comparables (0.27~s vs 0.27~s). 
\begin{table}[H]
...
%- - - - - - - - - - - - - - - - - - - - - -
\subsection{An\'alisis cinem\'atico global y estabilidad}
...
\begin{adjustwidth}{-\extralength}{0cm}
	...
	\begin{tabular}{p{2.5cm}p{1.6cm}p{1.8cm}p{2.2cm}p{1.0cm}p{1.0cm}p{0.8cm}p{0.8cm}p{3.5cm}}
		\toprule
		\textbf{Trabajo} & \textbf{Estrategia} & \textbf{Hardware} & \textbf{Valid.\ remota vs.\ presencial} & \textbf{Coste (USD)} & \textbf{Rec.\ limit.} & \textbf{Web} & \textbf{C\'od.\ ab.} & \textbf{Latencia / t.\ resp.\ (aprox.)} \\
		\midrule
		Este trabajo                               & Retrofitting   & ESP32 + RPi~4   & S\'i (cuantitativa) & $<$150      & S\'i    & S\'i    & S\'i    & Sellado temporal local en ESP32; latencia de red solo afecta visualizaci\'on ($\sim$decenas de ms en LAN) \\
		Lustig \textit{et al.}~\cite{r10}          & Retrofitting   & ESP32 + RPi     & No                  & $\sim$200   & Parcial & S\'i    & S\'i    & $\approx$\,220\,ms (comandos, 1 usuario); 200--400\,ms (video WebRTC); 0.35--2.01\,s seg\'un c\'amara \\
		P\'erez-Cham\'e \textit{et al.}~\cite{r11} & Nuevo hardware & ESP32           & No                  & $\sim$100   & S\'i    & No      & Parcial & NR \\
		Guerrero-Osuna \textit{et al.}~\cite{r4}   & Nuevo sistema  & Arduino + cloud & No                  & $>$500      & No      & S\'i    & No      & NR (pendiente en trabajos futuros; preset \textit{ultrafast} en YouTube) \\
		Viswanadh \textit{et al.}~\cite{r2}        & Retrofitting   & RPi + sensores  & No                  & $\sim$200   & S\'i    & Parcial & No      & NR \\
		Fuertes \textit{et al.}~\cite{r3}          & Nuevo sistema  & Embebido + nube & No                  & $>$500      & No      & S\'i    & No      & NR (bucle de adquisici\'on 100\,ms) \\
		\bottomrule
	\end{tabular}
\end{adjustwidth}
\end{table}
...
\subsection{Similitudes, divergencias y ventajas frente a la literatura}
