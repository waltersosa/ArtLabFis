...
En este marco, el presente trabajo se diferencia al proponer y validar un modelo de laboratorio híbrido de física basado en \textit{retrofitting} IoT. Para contextualizar nuestra contribución dentro de las arquitecturas modernas de IoT, se adopta el enfoque de capas propuesto por Dizdarevic y Jukan \cite{r42}, quienes destacan la importancia de integrar capacidades de computación en el borde (\textit{Edge}) y en la nube (\textit{Cloud}) para reducir la latencia en entornos educativos, una lección que hemos aplicado mediante el uso del dispositivo de control para el procesamiento en el borde. Asimismo, para asegurar la continuidad operativa y la accesibilidad universal, seguimos los principios de Azad \cite{r44} sobre el despliegue de laboratorios remotos basados en IoT, garantizando que el sistema sea robusto frente a desconexiones. Finalmente, la validación de nuestro sistema se alinea con la metodología de "gemelos digitales" descrita por Palmer \textit{et al.} \cite{r43}, utilizando la comparación directa entre los datos físicos del sensor (el "gemelo real") y el modelo teórico esperado, asegurando así una verificación cuantitativa rigurosa.
...
\begin{table}[H]
	...
	\begin{adjustwidth}{-\extralength}{0cm}
		...
		\begin{tabularx}{\fulllength}{>{\raggedright\arraybackslash}m{4.5cm}Xc}
			...
			\midrule
			Sistema experimental mec\'anico & Pista tipo Hot Wheels\textsuperscript{\textregistered} utilizada como riel de movimiento rectil\'ineo y carrito de baja fricci\'on adaptado para pr\'acticas de cinem\'atica & \cite{r9, r10} \\
			Sistema de sensado y adquisici\'on de datos & Cuatro sensores infrarrojos para detecci\'on de paso y medici\'on de tiempo & \cite{r11} \\
			Unidad de control y procesamiento IoT & Módulo de control DevKit (30 pines, USB-C, conectividad WiFi y Bluetooth integrada) & \cite{r12} \\
			Estructura mec\'anica y soporte f\'isico & Soportes, refuerzos y conjuntos mec\'anicos impresos en 3D para montaje de sensores, actuadores y elementos estructurales & \cite{r13, r14, r15, r16} \\
			Actuadores y control de movimiento & Servomotor SG90 (180\textdegree), motor paso a paso bipolar NEMA 17 y m\'odulo puente H L298N para control de movimiento & \cite{r20, r21, r22} \\
			Interfaz de usuario local & Pantalla LCD 20x4 con interfaz I\textsuperscript{2}C y pulsador mec\'anico para interacci\'on local & \cite{r23, r24} \\
			Comunicaci\'on y conectividad & Est\'andar inal\'ambrico IEEE 802.11 (WiFi), cable USB tipo C y cables Dupont para interconexi\'on el\'ectrica & \cite{r25, r28, r17} \\
			Sistema de alimentaci\'on y protecci\'on & Adaptador AC--DC 12 V / 1 A y caja pl\'astica de protecci\'on para proyectos electr\'onicos (135 $\times$ 75 $\times$ 40 mm) & \cite{r18, r19} \\
			Sistema de visualizaci\'on remota & Webcam Insta360 Link con resoluci\'on 4K y funciones de seguimiento asistidas por IA & \cite{r26} \\
			Servidor y gesti\'on del sistema IoT & Raspberry Pi 4 Model B para gesti\'on de comunicaciones, almacenamiento y monitoreo del sistema & \cite{r27} \\
			\bottomrule
		\end{tabularx}
	\end{adjustwidth}
\end{table}
...
\begin{table}[H]
	...
	\begin{adjustwidth}{-\extralength}{0cm}
		...
		\begin{tabularx}{\fulllength}{>{\raggedright\arraybackslash}m{3.5cm}X>{\centering\arraybackslash}m{3.5cm}c}
			...
			\midrule
			Comunicaci\'on IoT & EMQX, MQTTX Desktop & 5.10.2; 1.12.1 & \cite{r29, r30} \\
			Backend y servicios & Node.js (LTS), Express & 24.11.1; 5.1.0 & \cite{r31, r32} \\
			Persistencia de datos & MongoDB Server, MongoDB Driver, Mongoose & 8.2.2; 6.3.0; 8.5.1 & \cite{r33, r34} \\
			Frontend y visualizaci\'on & Next.js, React & 16.0; 19.2.0 & \cite{r35, r36} \\
			Desarrollo embebido & Arduino IDE & 2.3.4 & \cite{r37} \\
			Lenguajes y soporte & Python & 3.14.1 & \cite{r38} \\
			\bottomrule
		\end{tabularx}
	\end{adjustwidth}
\end{table}
...
\caption{Esquema del m\'odulo de control. Imagen generada con la asistencia de ChatGPT \cite{r41}.}
...
En este estudio no se trabaja con una poblaci\'on de sujetos humanos ni con unidades organizacionales, sino con un sistema f\'isico instrumentado cuyo comportamiento se eval\'ua desde una perspectiva t\'ecnica. Por este motivo, en lugar de definir una poblaci\'on y muestra en el sentido cl\'asico de la investigaci\'on cuantitativa, esta subsecci\'on se centra en describir el entorno experimental del laboratorio y la forma en que se planificaron los ensayos realizados sobre el montaje. En total, se ejecutaron 140 ensayos, correspondientes a 70 pruebas en modalidad presencial y 70 en modalidad remota.

El experimento se llev\'o a cabo en el laboratorio de f\'isica de la Pontificia Universidad Cat\'olica del Ecuador sede Esmeraldas, que dispone de una mesa de trabajo nivelada, acceso a red el\'ectrica regulada y conectividad de red local mediante un punto de acceso WiFi. En este espacio se instal\'o la pista tipo Hot Wheels\textsuperscript{\textregistered} \cite{r9}, el carrito de laboratorio de baja fricci\'on \cite{r10}, los cuatro sensores infrarrojos \cite{r11}, el módulo de control \cite{r12} y la c\'amara Insta360 Link \cite{r26}, conformando el montaje f\'isico del experimento de MRUA. La Raspberry~Pi~4 utilizada como servidor IoT \cite{r27} se ubic\'o en el mismo laboratorio y se conect\'o tanto al punto de acceso WiFi \cite{r28} como a la red cableada institucional.
...
El backend procesa los mensajes, los registra en MongoDB y los expone a la aplicaci\'on web, que a su vez presenta los datos y el video del experimento al usuario. Fue elaborada con la herramienta de diagramaci\'on asistida por IA Eraser\cite{r39}}
...
En la validación técnica del prototipo no se trabajó con variables independientes y dependientes manipuladas en el sentido clásico, sino con métricas cuantitativas basadas en los tiempos de detección registrados por los sensores distribuidos a lo largo de la pista experimental. El objetivo fue cuantificar, por un lado, la precisión con la que cada sensor detecta el paso del carrito en comparación con un tiempo de referencia y, por otro, la coherencia entre los tiempos medidos por los sensores y los tiempos que el propio sistema calcula internamente a partir del modelo cinemático del experimento.

El modelo utilizado por el sistema se basa en las ecuaciones del movimiento rectilíneo uniformemente acelerado (MRUA), considerando parámetros previamente establecidos para el experimento, tales como la aceleración nominal del carrito y las posiciones fijas de los sensores a lo largo de la pista. A partir de estas ecuaciones, el sistema estima los tiempos esperados de paso del carrito por cada sensor, los cuales son utilizados como referencia interna para la evaluación de la coherencia del sistema de medición.

En primer lugar, se consideró el error relativo del tiempo de detección por sensor. Para cada sensor \(i\), se comparó el tiempo de paso medido por el sensor (\(t_i^{\text{medido}}\)) con un tiempo de referencia (\(t_i^{\text{ref}}\)), obtenido mediante una medición manual realizada con un cronómetro digital durante la ejecución del experimento en modalidad presencial, bajo condiciones controladas. El error relativo se expresó como un porcentaje según la siguiente ecuación:
\[
E^{\%}_{t,i} = \frac{\bigl| t_i^{\text{medido}} - t_i^{\text{ref}} \bigr|}{t_i^{\text{ref}}} \times 100.
\]

Cuando el sensor detecta correctamente el paso del carrito, se espera que \(t_i^{\text{medido}}\) y \(t_i^{\text{ref}}\) sean muy similares y que el error relativo tome valores cercanos a cero. Valores elevados de \(E^{\%}_{t,i}\) indican situaciones en las que el sensor no detecta adecuadamente el evento, como activaciones tardías, detecciones incompletas o pérdida del evento, lo que afecta directamente la calidad de los datos experimentales.

En segundo lugar, se utilizó el error entre el tiempo medido y el tiempo calculado por el sistema para la modalidad híbrida. En este caso, para cada sensor \(i\), se comparó el tiempo de paso medido directamente a partir de la señal del sensor infrarrojo (\(t_i^{\text{medido}}\)) con el tiempo que el sistema calcula internamente (\(t_i^{\text{calculado}}\)) a partir del modelo cinemático del MRUA configurado para el experimento. Los tiempos calculados se obtuvieron utilizando las ecuaciones del MRUA y los parámetros previamente establecidos, tales como la aceleración del sistema y las distancias conocidas entre los sensores.

En el sistema propuesto, los tiempos son medidos localmente por el módulo de control utilizando su temporizador interno de alta resolución. El conteo del tiempo se inicia en el instante en que el sensor S1 detecta el paso del carrito y se detiene cuando el carrito es detectado por cada uno de los sensores posteriores. De este modo, el tiempo asociado a cada sensor se obtiene a partir de la diferencia entre marcas de tiempo registradas por el propio módulo de control, sin depender de la comunicación en red ni de los servicios remotos. Los valores de tiempo calculados y medidos son posteriormente transmitidos mediante el protocolo MQTT para su almacenamiento y análisis.

El error relativo entre el tiempo medido y el tiempo calculado por el sistema se definió como:
\[
E^{\%}_{\text{calc},i} = \frac{\bigl| t_i^{\text{medido}} - t_i^{\text{calculado}} \bigr|}{t_i^{\text{calculado}}} \times 100.
\]

Esta métrica permite identificar discrepancias entre la señal real capturada por el sensor y el comportamiento esperado según el modelo cinemático del MRUA, las cuales pueden deberse a errores de configuración, retardos en la adquisición de datos o limitaciones propias del esquema de detección utilizado.

En conjunto, estas métricas proporcionan una medida directa de la calidad de la detección de los sensores y de la coherencia interna de los tiempos manejados por el sistema durante la ejecución del experimento, considerando el intervalo comprendido entre el inicio del movimiento del carrito y su llegada al punto final del recorrido instrumentado.

%-------------------------------------------------

\subsection{M\'etodos de an\'alisis de datos}

Los datos registrados por el prototipo IoT (modalidad h\'ibrida) y por el montaje presencial de referencia se exportaron en forma de archivos de texto estructurados, que contienen las marcas de tiempo asociadas al paso del carrito por cada sensor y la configuraci\'on de cada ensayo. Estos registros se organizaron en hojas de c\'alculo y posteriormente se procesaron mediante scripts desarrollados en Python~3.14.1\cite{r38}.

El an\'alisis se centr\'o en las variables definidas en la subsecci\'on anterior: tiempos de paso por sensor, velocidades promedio entre pares de sensores y aceleraci\'on promedio del carrito en el tramo instrumentado. A partir de estas variables se calcularon, para cada sensor y cada serie de ensayos, las m\'etricas de error absoluto y relativo entre la modalidad h\'ibrida y la modalidad presencial, as\'i como medidas b\'asicas de variabilidad (media y desviaci\'on est\'andar) para valorar la consistencia de las mediciones.

En una primera etapa, los datos de ambas modalidades se integraron en tablas comparativas que agrupan, por ensayo, los tiempos de paso registrados por el sistema h\'ibrido y por el montaje presencial, junto con los errores correspondientes. En una segunda etapa, se generaron gr\'aficos de dispersi\'on, histogramas y diagramas de caja para visualizar la proximidad entre las mediciones obtenidas en ambas modalidades y detectar posibles desviaciones sistem\'aticas del prototipo.

Los resultados num\'ericos de estas comparaciones, as\'i como las tablas y gr\'aficos derivados del procesamiento en Python~3.14.1, se presentan y discuten en la secci\'on de Resultados, donde se analiza en detalle el desempe\~no del laboratorio h\'ibrido frente al montaje presencial de referencia.
...
This section presents the results obtained from the experimental validation of the proposed IoT-based retrofitting system. The analysis focuses on the temporal accuracy of the sensors, the consistency of the kinematic measurements, and the comparison between the remote (IoT) and presencial (traditional) modalities. The data set comprises \(N=140\) independent trials (70 remote and 70 presencial).

\subsection{Correlation Analysis Methodology}

To evaluate the degree of linear association between the measurements obtained in the presencial modality and those obtained in the remote modality, the Pearson correlation coefficient (\(r\)) was calculated for each sensor. It is important to note that the trials in both modalities were performed as independent events; therefore, no direct paired physical correlation is expected between the \(i\)-th presencial trial and the \(i\)-th remote trial unless systematic underlying variables (e.g., track friction variations over time) influence both datasets similarly.

The correlation coefficient is calculated using the following equation:
...
Where:
\begin{itemize}
    \item \(x_i\): Time measured in the presencial modality for trial \(i\).
    \item \(y_i\): Time measured in the remote modality for trial \(i\).
    \item \(\bar{x}, \bar{y}\): Arithmetic means of the presencial and remote datasets, respectively.
    \item \(n\): Total number of trials per modality (approx. 70).
\end{itemize}

This equation is applied independently to the data recorded by each sensor (S1 to S4) to analyze the behavior of the system at different stages of the movement.

\subsection{Sensor S1: Start of Movement}

Sensor S1 corresponds to the starting point of the movement (\(t=0\)). In both modalities, this sensor acts as the temporal trigger for the experiment; therefore, the recorded time is systematically zero or very close to it.
...
\begin{figure}[H]
    ...
    \caption{Correlation graph for Sensor S1 (Remote vs. Presencial).}
    \label{fig:corr_s1}
\end{figure}

As shown in Figure \ref{fig:corr_s1}, the data points are concentrated at the origin. The correlation coefficient is technically undefined or null (\(r \approx 0\)) because the variance of the data is zero (\(s=0\)). This behavior is physically expected and confirms that S1 functions correctly as the reference synchronization point for both the manual stopwatch and the digital module of control. There is no jitter or experimental noise affecting this initial state.

\subsection{Sensor S2: First Intermediate Section}

Sensor S2 is located at the end of the first section of the track. The results in Table \ref{tab:s2_results} show a high consistency between the means of both modalities (1.20 s vs 1.21 s), with an identical standard deviation.
...
\begin{figure}[H]
    ...
    \caption{Correlation graph for Sensor S2 (Remote vs. Presencial).}
    \label{fig:corr_s2}
\end{figure}

The graph in Figure \ref{fig:corr_s2} presents a dispersed distribution of points. The calculated correlation coefficient is low (\(r < 0.3\)), classifying the correlation as null or weak. This result is interpreted physically by the independence of the trials: since the "presencial trial 1" and "remote trial 1" are distinct physical events separated in time, they are subject to different random fluctuations (initial release friction, slight air resistance variations). The lack of correlation validates that the measurement error is random rather than systematic; the IoT system does not introduce a bias that approximates or distances it essentially from the manual measurement, but simply captures the natural variability of the MRUA phenomenon.

\subsection{Sensor S3: Second Intermediate Section}

Sensor S3 captures the movement at a higher velocity. Table \ref{tab:s3_results} indicates a slight increase in the mean time for the remote modality (1.68 s) compared to the presencial one (1.66 s), but the standard deviation remains comparable, actually being lower in the remote case.
...
\begin{figure}[H]
    ...
    \caption{Correlation graph for Sensor S3 (Remote vs. Presencial).}
    \label{fig:corr_s3}
\end{figure}

Figure \ref{fig:corr_s3} again shows a cloud of points with low correlation. The weak value of \(r\) is attributed to the accumulating experimental noise. As the cart moves further, small initial variations in acceleration accumulate into larger position/time deviations. The fact that the remote system shows a lower standard deviation (\(s=0.22\) s vs \(s=0.25\) s) suggests that the automated detection of the module of control is less susceptible to the "reaction time jitter" that affects human timing with a stopwatch, providing a more precise characterization of the cart's transit.

\subsection{Sensor S4: End of Track}

Sensor S4 represents the final measurement point, where the velocity is maximum. The results highlight a significant difference in the standard deviation.
...
\begin{figure}[H]
    ...
    \caption{Correlation graph for Sensor S4 (Remote vs. Presencial).}
    \label{fig:corr_s4}
\end{figure}

The analysis of Figure \ref{fig:corr_s4} reveals the most critical insight. The presencial measurements show a very high dispersion (\(s=1.16\) s), likely due to the difficulty of manually timing a fast-moving object at the end of the track. In contrast, the remote IoT system maintains a tight precision (\(s=0.29\) s). The low correlation here is driven by this discrepancy in variance; the manual measurements are dominated by human error (noise), while the remote measurements cluster around the true kinematic value. Physically, this demonstrates that the IoT retrofitting effectively filters out the observation error, providing a cleaner dataset for MRUA analysis despite the lack of linear correlation with the noisy manual control group.
...
A pesar del desempeño satisfactorio del prototipo, se identifican limitaciones intrínsecas en el alcance de la investigación. En primer lugar, aunque la estabilidad fue alta, el sistema sigue dependiendo de la disponibilidad continua del servidor MQTT para la transmisión de datos en tiempo real. En segundo lugar, el número de ensayos realizados (n=70 por modalidad), aunque robusto, se limita a un entorno controlado de laboratorio universitario. Asimismo, el experimento se desarrolla bajo condiciones mecánicas donde factores como la fricción residual del carrito y la alineación micrométrica de los sensores ópticos introducen fuentes de error físico que son independientes de la instrumentación IoT.
...
El presente estudio ha validado con éxito la eficacia del \textit{retrofit} de laboratorios de física mediante tecnologías IoT de bajo coste. A través de una extensa campaña experimental con 140 ensayos (70 presenciales y 70 remotos), se demostró que la arquitectura propuesta basada en el módulo de control y sensores infrarrojos es capaz de replicar la dinámica del MRUA con una fidelidad comparable, e incluso superior en términos de estabilidad, a la del montaje tradicional.
...
Available online: \url{https://www.vishay.com/en/product/83751} (accessed on 5 December 2025).

\bibitem{r12}
Espressif Systems.
\textit{ESP32-DevKitC Development Board}.
Available online: \url{https://docs.espressif.com/projects/esp-idf/en/latest/hw-reference/esp32/get-started-devkitc.html} (accessed on 5 December 2025).
...
\bibitem{r20}
Tower Pro.
\textit{SG90 Micro Servo Motor (180\textdegree)}.
Available online: \url{https://www.towerpro.com.tw/product/sg90-7/} (accessed on 5 December 2025).

\bibitem{r21}
StepperOnline.
\textit{NEMA 17 Stepper Motor Bipolar}.
Available online: \url{https://www.omc-stepperonline.com/nema-17-stepper-motor} (accessed on 5 December 2025).

\bibitem{r22}
STMicroelectronics.
\textit{L298N Dual H-Bridge Motor Driver Module}.
Available online: \url{https://www.st.com/en/motor-drivers/l298.html} (accessed on 5 December 2025).

\bibitem{r23}
Generic Electronics Suppliers.
\textit{LCD 20x4 Display with I\textsuperscript{2}C Interface}.
Available online: \url{https://www.adafruit.com/product/198} (accessed on 5 December 2025).
...
Available online: \url{https://www.usb.org/documents} (accessed on 5 December 2025).

\bibitem{r26}
Insta360.
\textit{Insta360 Link---4K AI Webcam}.
Available online: \url{https://www.insta360.com/product/insta360-link} (accessed on 5 December 2025).

\bibitem{r27}
Raspberry Pi Ltd.
\textit{Raspberry Pi 4 Model B}.
Available online: \url{https://www.raspberrypi.com/products/raspberry-pi-4-model-b} (accessed on 5 December 2025).

\bibitem{r28}
IEEE.
\textit{IEEE 802.11 Wireless LAN Standard Family}.
Available online: \url{https://en.wikipedia.org/wiki/IEEE_802.11ac} (accessed on 5 December 2025).

\bibitem{r29}
EMQ Technologies Co., Ltd.
\textit{EMQX---MQTT Platform for IoT Data Streaming}.
Available online: \url{https://www.emqx.com/en} (accessed on 5 December 2025).

\bibitem{r30}
EMQ Technologies Co., Ltd.
\textit{MQTTX: Your All-in-One MQTT Client Toolbox}.
Available online: \url{https://mqttx.app} (accessed on 5 December 2025).

\bibitem{r31}
OpenJS Foundation.
\textit{Node.js---Run JavaScript Everywhere}.
Available online: \url{https://nodejs.org/en} (accessed on 5 December 2025).

\bibitem{r32}
OpenJS Foundation.
\textit{Express---Fast, Unopinionated, Minimalist Web Framework for Node.js}.
Available online: \url{https://expressjs.com} (accessed on 5 December 2025).

\bibitem{r33}
MongoDB, Inc.
\textit{MongoDB Community Server}.
Available online: \url{https://www.mongodb.com/products/self-managed/community-edition} (accessed on 5 December 2025).

\bibitem{r34}
Mongoose.
\textit{Mongoose ODM for MongoDB}.
Available online: \url{https://mongoosejs.com} (accessed on 5 December 2025).

\bibitem{r35}
Vercel, Inc.
\textit{Next.js---The React Framework for the Web}.
Available online: \url{https://nextjs.org} (accessed on 5 December 2025).

\bibitem{r36}
Meta Platforms, Inc.
\textit{React---A JavaScript Library for Building User Interfaces}.
Available online: \url{https://react.dev} (accessed on 5 December 2025).

\bibitem{r37}
Arduino AG.
\textit{Arduino IDE 2.x Software}.
Available online: \url{https://www.arduino.cc/en/software} (accessed on 5 December 2025).

\bibitem{r38}
Python Software Foundation.
\textit{Python Programming Language}.
Available online: \url{https://www.python.org} (accessed on 5 December 2025).

\bibitem{r39}
Eraser Labs, Inc.
\textit{Eraser---AI Co-Pilot for Technical Design and Documentation}.
Available online: \url{https://www.eraser.io} (accessed on 5 December 2025). Image generated with the assistance of Gemini.
