...
\abstract{La actualización de laboratorios de física en instituciones con presupuesto limitado suele enfrentarse a una tensión práctica: se necesita ampliar el acceso y, al mismo tiempo, conservar equipos que todavía son funcionales. En este trabajo se diseña y valida un laboratorio remoto para el estudio del Movimiento Rectilíneo Uniformemente Acelerado (MRUA) mediante una estrategia de \textit{retrofitting} IoT sobre infraestructura existente. La solución integra un ESP32 (módulo de control), sensores infrarrojos de paso y una interfaz web para operación y monitoreo en tiempo real. La validación se desarrolló con 70 ensayos independientes (35 remotos y 35 presenciales), comparando tiempos de detección y magnitudes cinemáticas derivadas entre ambas modalidades. Los resultados muestran que el sistema remoto reproduce el comportamiento del montaje de referencia con diferencias acotadas y estabilidad operativa durante la campaña experimental. En conjunto, los hallazgos respaldan que el \textit{retrofitting} IoT constituye una alternativa técnicamente viable y escalable para fortalecer la enseñanza experimental en escenarios híbridos sin exigir una renovación completa del laboratorio.}
...
Los laboratorios de f\'isica siguen siendo el espacio donde los modelos te\'oricos se contrastan con evidencia medible, y por ello conservan un papel central en la formaci\'on de ingenier\'ia. No obstante, buena parte de estos entornos fue dise\~nada para una operaci\'on estrictamente presencial: instrumentaci\'on local, registro manual y baja trazabilidad de datos. En paralelo, la digitalizaci\'on de la infraestructura cient\'ifica y las arquitecturas ciberf\'isicas han abierto la posibilidad de operar experimentos reales de forma remota, con supervisi\'on y captura de datos en l\'inea \cite{r1}. La experiencia acumulada tras la pandemia de COVID-19 hizo m\'as visible esta necesidad y aceler\'o la adopci\'on de laboratorios conectados a servicios en la nube \cite{r2}.

En Am\'erica Latina, esta transici\'on suele tropezar con una limitaci\'on concreta: los laboratorios funcionan con equipamiento que a\'un es "\'util", pero que no fue concebido para automatizaci\'on ni acceso remoto. El caso de la Pontificia Universidad Cat\'olica del Ecuador, sede Esmeraldas, refleja esa realidad: existen montajes funcionales para cinem\'atica (pista y carrito de baja fricci\'on), aunque sin una capa digital que permita controlar, registrar y compartir el experimento a distancia. Frente a este escenario, el \textit{retrofitting} con tecnolog\'ias IoT de bajo coste no se plantea como reemplazo total del laboratorio, sino como una estrategia de modernizaci\'on progresiva y viable \cite{r3,r4}.
...
\begin{table}[H]
	...
	\begin{adjustwidth}{-\extralength}{0cm}
		...
		\begin{tabularx}{\fulllength}{clXccc}
			...
			7  & L298N – ENA    & Control de aceleraci\'on del motor (MRUA) & GPIO 14 & PWM                            & 5 V / GND    \\
			8  & L298N – IN1    & Direcci\'on del motor DC                  & GPIO 27 & Salida digital                 & —            \\
			9  & L298N – IN2    & Direcci\'on del motor DC                  & GPIO 26 & Salida digital                 & —            \\
			10 & LCD 20$\times$4 – SDA & Comunicaci\'on I2C (datos)                & GPIO 21 & I2C                            & 5 V / GND    \\
			11 & LCD 20$\times$4 – SCL & Comunicaci\'on I2C (reloj)                & GPIO 22 & I2C                            & 5 V / GND    \\
...
%-------------------------------------------------
\subsection{M\'etricas de an\'alisis y comparaci\'on}

En la validaci\'on t\'ecnica del prototipo, el an\'alisis se orient\'o a comparar la proximidad entre modalidades en t\'erminos de tendencia central y dispersi\'on para cada sensor (S1--S4). Dado que los ensayos presenciales y remotos fueron ejecuciones f\'isicas independientes, no se emple\'o correlaci\'on ensayo-a-ensayo como criterio principal de desempe\~no.

Como criterio de comparaci\'on entre modalidades, se utilizaron tres indicadores descriptivos: diferencia de medias, error relativo porcentual y diferencia de desviaciones est\'andar.

Los indicadores se definen mediante las siguientes ecuaciones:

\begin{equation}
\Delta \bar{t} = \bar{t}_{\mathrm{rem}} - \bar{t}_{\mathrm{pres}}
\label{eq:delta_media}
\end{equation}

\begin{equation}
E_{\%} = \frac{|\bar{t}_{\mathrm{rem}} - \bar{t}_{\mathrm{pres}}|}{\bar{t}_{\mathrm{pres}}}\times 100
\label{eq:error_relativo}
\end{equation}

\begin{equation}
\Delta s = s_{\mathrm{rem}} - s_{\mathrm{pres}}
\label{eq:delta_sd}
\end{equation}

donde \(\bar{t}_{\mathrm{pres}}\) y \(\bar{t}_{\mathrm{rem}}\) son las medias de tiempo en modalidad presencial y remota, y \(s_{\mathrm{pres}}\), \(s_{\mathrm{rem}}\) sus respectivas desviaciones est\'andar.
...
En una primera etapa, los datos de ambas modalidades se integraron en tablas comparativas para cada sensor. En una segunda etapa, se calcularon estad\'isticos descriptivos (media y desviaci\'on est\'andar) y los indicadores \(\Delta \bar{t}\), \(E_{\%}\) y \(\Delta s\). Finalmente, se generaron gr\'aficos de dispersi\'on e histogramas con fines descriptivos para visualizar la proximidad global entre mediciones y detectar posibles desviaciones del prototipo.
...
Esta sección presenta los resultados obtenidos de la validación experimental del sistema de \textit{retrofitting} basado en IoT propuesto. El análisis se centra en la estabilidad de la captura de datos y en la concordancia descriptiva entre las modalidades remota (IoT) y presencial (tradicional). El conjunto de datos comprende \(N=70\) ensayos independientes (35 remotos y 35 presenciales).
...
\begin{figure}[H]
    ...
    \caption{Comparaci\'on descriptiva de tiempos para el Sensor S1 (Remota vs. Presencial).}
    \label{fig:corr_s1}
\end{figure}

Como se muestra en la Figura~\ref{fig:corr_s1}, los puntos de datos se concentran en el origen. En ambas modalidades, la media y la desviaci\'on est\'andar son \(0.00\) s, por lo que la diferencia entre modalidades es nula (\(\Delta \bar{t}=0.00\) s; \(\Delta s=0.00\) s). Este comportamiento es f\'isicamente esperado y confirma que S1 funciona correctamente como punto de referencia de sincronizaci\'on para el cronometraje manual y para el m\'odulo de control digital.
...
\begin{figure}[H]
    ...
    \caption{Comparaci\'on descriptiva de tiempos para el Sensor S2 (Remota vs. Presencial).}
    \label{fig:corr_s2}
\end{figure}

La Figura~\ref{fig:corr_s2} muestra una dispersi\'on coherente con ensayos independientes. La comparaci\'on descriptiva reporta \(\Delta \bar{t}=0.02\) s (1.71\% respecto a presencial) y \(\Delta s=0.01\) s, lo que indica proximidad entre modalidades tanto en tendencia central como en variabilidad para este tramo de la pista.
...
\begin{figure}[H]
    ...
    \caption{Comparaci\'on descriptiva de tiempos para el Sensor S3 (Remota vs. Presencial).}
    \label{fig:corr_s3}
\end{figure}

La Figura~\ref{fig:corr_s3} mantiene el patr\'on de proximidad entre modalidades. Para este sensor se obtiene \(\Delta \bar{t}=0.03\) s (1.81\%) y \(\Delta s=0.01\) s. Aunque la dispersi\'on aumenta respecto a S2 por acumulaci\'on de variaciones mec\'anicas a lo largo del recorrido, la similitud de medias y desviaciones est\'andar respalda un desempe\~no comparable entre el registro remoto y el presencial.
...
\begin{figure}[H]
    ...
    \caption{Comparaci\'on descriptiva de tiempos para el Sensor S4 (Remota vs. Presencial).}
    \label{fig:corr_s4}
\end{figure}

El an\'alisis de la Figura~\ref{fig:corr_s4} confirma el patr\'on observado en los sensores anteriores. Las mediciones presenciales y remotas muestran dispersi\'on comparable (\(s=0.28\) s y \(s=0.29\) s) y tiempos medios pr\'acticamente id\'enticos (2.05 s vs 2.04 s), con \(\Delta \bar{t}=-0.01\) s (0.49\%) y \(\Delta s=0.01\) s. Este acuerdo al final del recorrido respalda que el sistema de \textit{retrofitting} IoT reproduce la medici\'on temporal del montaje tradicional con estabilidad adecuada para uso acad\'emico.
...
\subsection{Interpretaci\'on de resultados temporales y cinem\'aticos}

Los resultados obtenidos demuestran que el sistema de \textit{retrofitting} IoT es capaz de capturar la din\'amica del experimento de MRUA con fidelidad comparable al montaje presencial tradicional. En los sensores S2--S4, las diferencias de medias entre modalidades se mantienen entre 0.01 s y 0.03 s, y las diferencias de dispersi\'on en torno a 0.01 s. Estos valores indican que la modalidad remota conserva estabilidad de medici\'on similar a la referencia presencial. Adem\'as, el sellado temporal local en el m\'odulo de control reduce el impacto de la latencia de red sobre el registro de eventos f\'isicos.

\subsection{Implicaciones para el \textit{retrofitting} IoT en laboratorios de f\'isica}
...
\subsection{Comparaci\'on con trabajos relacionados}
...
Este estudio permitió comprobar, en condiciones de laboratorio reales, que un enfoque de \textit{retrofitting} IoT puede modernizar un experimento clásico de MRUA sin desmantelar la infraestructura existente. A partir de 70 ensayos independientes (35 presenciales y 35 remotos), la arquitectura implementada con módulo de control, sensores infrarrojos e interfaz web mostró un comportamiento consistente y técnicamente comparable al del montaje de referencia.

En términos de desempeño, la operación remota no introdujo una pérdida relevante de precisión en las variables temporales analizadas. Más bien, las métricas observadas indican una estabilidad suficiente para uso académico continuo, con una captura automática de datos que mejora la trazabilidad del experimento frente al registro exclusivamente manual. Además, la integración de video y visualización en línea aportó valor didáctico al facilitar la relación entre el fenómeno físico observado y los datos registrados.

En síntesis, la contribución principal de este trabajo es mostrar que la transformación digital de laboratorios de física puede abordarse de forma gradual, coste-efectiva y replicable. Como línea de continuidad, resulta pertinente ampliar la validación a otros montajes de mecánica y estudiar de manera explícita el impacto de la latencia de red y de la calibración periódica de sensores en escenarios de uso prolongado.
...
\textit{arXiv} \textbf{2025}, arXiv:2504.11363. 
\href{https://arxiv.org/abs/2504.11363}{arXiv:2504.11363}.
